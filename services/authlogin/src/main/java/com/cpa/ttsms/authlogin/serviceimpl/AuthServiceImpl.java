...
//	private PublicKey publicKey;
	...
	public Object getServerPublicKey() throws IOException {
		LOGGER.info("Getting Server public key from file");

		try (InputStream inputStream = getClass().getClassLoader().getResourceAsStream(PUBLIC_KEY_FILE_NAME)) {
			if (inputStream == null) {
				throw new IOException("Resource not found: " + PUBLIC_KEY_FILE_NAME);
			}

			// Read public key from the input stream and wrap it in KeyDTO object
			String serverPublicKey = new String(inputStream.readAllBytes());
			KeyDTO serverPublicKeyObject = new KeyDTO(serverPublicKey);
			System.out.println("key is here  + " + serverPublicKey);
	...
	@Override
	public AuthKey getServerRandomString(int keyId) {
